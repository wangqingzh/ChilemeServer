...
public interface CommentMapper {
     ...
     */
    @Select("SELECT cm.post_id AS postId, cm.content AS comment, cm.from_uid AS fromUid, cm.to_uid AS toUid, " +
            "       cm.create_time AS writeTime, ui.nick_name AS userName, ui.avatar_url AS userAvatar, " +
            "       (SELECT  COUNT(*) FROM my_like my WHERE  my.type = 1 AND my.status = true AND my.type_id = cm.post_id) AS likeNumber, " +
            "       IFNULL((SELECT  status FROM my_like my WHERE  my.type = 1 AND my.user_id = ${userId}), 0) AS likeStatus " +
